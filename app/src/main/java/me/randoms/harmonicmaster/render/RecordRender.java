...
public class RecordRender implements GLSurfaceView.Renderer {
    ...
    public RecordRender(GameActivity mContext, JSONObject musicJson, ToneModel[] bottomToneList){
    ...
    public void onSurfaceCreated(GL10 unused, EGLConfig config) {
        ...
        for(int count = 0;count<music.length();count ++){
            ...
            if(bottomToneList[sound.getInt("name")-1] != null){
    ...
    public void onDrawFrame(GL10 unused) {
        ...
        if( - musicLength * 128 /1000f > (cameraPoxYinPix + 720/2)){
        ...
        for(int count = 0; count < toneList.size();count ++){
            ...
            if(currentTone.getBottom() > cameraPoxYinPix + (checkLinePosYinPix - 720/2)
                    && currentTone.getTop() < cameraPoxYinPix + (checkLinePosYinPix - 720/2)){
                mContext.updateCurrentTone(currentTone.getTonename());
                noToneFlag = false;
            }
            else{
